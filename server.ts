...
function generateHeuristicForecast(params: any) {
  ...
  return {
    ...
    keyActionItems: [
      ...
      'Stage 200 filled 18.9L bottles at Nyakato Bottling Plant loading bay by 06:30 AM for early morning corporate deliveries.',
      'Consolidate residential Capripoint & Kirumba lakefront routes into a dedicated afternoon delivery run (13:30 - 17:00).',
      'Schedule truck T 119 CZZ routine mechanical inspection on Sunday when commercial route demand dips by 52%.',
    ],
    shiftStaggeringPlan:
      'Shift A (06:45 - 14:00): 65% of drivers deployed to Nyakato, Buzuruga & CBD commercial zones. Shift B (11:00 - 18:30): 35% of drivers for secondary Capripoint hotel orders and Nyegezi residential dispenser refills.',
    fleetDeploymentAdvice:
      'Deploy 3.5-ton Isuzu vehicles for Nyakato & Buzuruga high-volume retail drops (15+ bottles/stop); use agile Toyota Hiace vans for Capripoint hill and Makoroboi market alleys.',
    riskMitigation:
      'Maintain an emergency buffer stock of 100 sealed bottles at Nyakato Central Depot to fulfill urgent Lake Victoria hotel replenishments during peak afternoon tourist check-ins.',
    source: 'logistics_engine',
  };
}

// In-memory fleet locations store for real-time supervisor & dispatcher map (Mwanza, Tanzania)
const activeFleetLocations = new Map<string, any>();

// Clean baseline fleet in Mwanza, Tanzania
const cleanMwanzaFleet = [
  {
    userId: 'drv-001',
    ...
    batteryLevel: 91,
    truckStock: { bottles18_9L: 26, bottles13L: 12 },
    totalStopsToday: 15,
  ...
  },
  {
    ...
    batteryLevel: 78,
    truckStock: { bottles18_9L: 42, bottles13L: 18 },
    totalStopsToday: 14,
  ...
  },
  {
    ...
    batteryLevel: 88,
    truckStock: { bottles18_9L: 14, bottles13L: 6 },
    totalStopsToday: 16,
    completedStopsToday: 12,
  },
];

cleanMwanzaFleet.forEach((f) => activeFleetLocations.set(f.userId, f));

// Real-Time Field Geolocation Endpoints (Mwanza, Tanzania)
app.get('/api/field-locations', (req, res) => {
  const list = Array.from(activeFleetLocations.values());
  res.json(list);
});

app.post('/api/field-locations/reset-mwanza', (req, res) => {
  activeFleetLocations.clear();
  cleanMwanzaFleet.forEach((f) => activeFleetLocations.set(f.userId, { ...f, updatedAt: Date.now() }));
  res.json({ status: 'ok', message: 'Fleet reset to clean Mwanza Tanzania baseline', fleet: Array.from(activeFleetLocations.values()) });
});
